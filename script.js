// Keep your existing script features; this is a full script with fixes + category filter

// Mobile nav
const hamb=document.querySelector('.hamburger');const nav=document.getElementById('nav');
if(hamb&&nav){
  hamb.addEventListener('click',()=>{const e='true'===hamb.getAttribute('aria-expanded');hamb.setAttribute('aria-expanded',String(!e)),nav.classList.toggle('open',!e)});
...
(function(){
  ...
  function draw(){
    ...
    for(const p of particles){
      ...
      p.tw+=p.tc; const alpha=.24+.22*Math.sin(p.tw);
  ...
  window.addEventListener('resize',resize,{passive:!0}); resize(); draw();
})();
...
// Expected markup (inside #gallery): <div class="cat-nav"><button class="chip" data-cat="all">הכל</button>...</div>
(function(){
  const chips = Array.from(document.querySelectorAll('.cat-nav .chip'));
  ...
  function apply(cat){
    ...
    items.forEach(it=>{
      const itemCat = it.dataset.cat || 'all';
  ...
  chips.forEach(chip=> chip.addEventListener('click', ()=>{
    ...
    apply(cat);
    // keep URL hint (so refresh keeps choice)
    const url = new URL(location.href);
    url.searchParams.set('cat', cat);
    history.replaceState(null,'',url.toString());
    // bring gallery into view
  ...
  const exists = chips.some(c=>c.dataset.cat===initial);
  apply(exists ? initial : 'all');
})();
