...
(function(){
  ...
  const ctx=canvas.getContext('2d'); let w=0,h=0,d=1,p=[];
  function R(){
    d=Math.min(window.devicePixelRatio||1,1.75);
    const vw=Math.max(document.documentElement.clientWidth,window.innerWidth||0);
    const vh=Math.max(document.documentElement.clientHeight,window.innerHeight||0);
    w=canvas.width=Math.floor(vw*d); h=canvas.height=Math.floor(vh*d);
    canvas.style.width=vw+'px'; canvas.style.height=vh+'px';
    const A=vw*vh, B=A<280000?60:A<810000?110:180;
  ...
  function D(){
    ...
    for(const o of p){
      o.x+=Math.cos(o.a)*o.s; o.y+=.5*Math.sin(o.a)*o.s; o.a+=(Math.random()-.5)*.02;
      if(o.x<0)o.x=w; else if(o.x>w)o.x=0; if(o.y<0)o.y=h; else if(o.y>h)o.y=0;
      o.tw+=o.tc; const a=.22+.22*Math.sin(o.tw);
      const g=ctx.createRadialGradient(o.x,o.y,0,o.x,o.y,6*o.r);
      g.addColorStop(0,`rgba(255,255,255,${a})`); g.addColorStop(1,'rgba(255,255,255,0)');
      ctx.fillStyle=g; ctx.beginPath(); ctx.arc(o.x,o.y,6*o.r,0,2*Math.PI); ctx.fill();
    }
    requestAnimationFrame(D);
  }
  window.addEventListener('resize',R,{passive:true}); R(); D();
})();
...
;(function(){
  ...
  let idx=0;
  function open(i){ idx=i; const el=items[idx]; img.src=el.getAttribute('data-src') || el.querySelector('img')?.src || ''; lb.setAttribute('aria-hidden','false'); document.body.style.overflow='hidden'; }
  function close(){ lb.setAttribute('aria-hidden','true'); document.body.style.overflow=''; }
  function prev(){ open((idx-1+items.length)%items.length) } function next(){ open((idx+1)%items.length) }
  items.forEach((btn,i)=>btn.addEventListener('click',()=>open(i))); closeBtn.addEventListener('click',close); prevBtn.addEventListener('click',prev); nextBtn.addEventListener('click',next);
  lb.addEventListener('click',e=>{ if(e.target===lb) close(); });
  document.addEventListener('keydown',e=>{ if(lb.getAttribute('aria-hidden')==='true') return; if(e.key==='Escape') close(); if(e.key==='ArrowLeft') prev(); if(e.key==='ArrowRight') next(); });
})();
